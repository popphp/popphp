...
class Gmagick extends AbstractRasterImage
{
     ...
     * image file on disk, or a new image file using the Gmagick extension.
     *
     * @param  string $img
     * @param  int    $w
     * @param  int    $h
    ...
    {
        // Check to see if Gmagick is installed.
        if (!self::isInstalled()) {
            throw new Exception('Error: The Gmagick extension must be installed to use the Gmagick adapter.');
        }
        ...
        $gmagickVersion = (new \Gmagick())->getVersion();
        $versionString  = trim(substr($gmagickVersion['versionString'], 0, stripos($gmagickVersion['versionString'], 'http://')));
    ...
    {
        return self::isAvailable('gmagick');
    }
    ...
    public function create($width, $height, $image = null)
    {
        ...
        $this->resource->newImage($width, $height, 'white');
    ...
    public function load($image)
    {
        ...
        $this->resource = new \Gmagick($image);
    ...
    public function save($to = null)
    {
        ...
        if (null !== $this->quality) {
            $this->resource->setCompressionQuality($this->quality);
        }
    ...
    public function getColor(array $color, $opacity = 100)
    {
        ...
        return new \GmagickPixel('rgba(' . $r . ',' . $g . ',' . $b . ',' . (int)$opacity . ')');
    }
    ...
    {
        $formats = (new \Gmagick())->queryFormats();
