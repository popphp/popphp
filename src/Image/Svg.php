...
class Svg
{
    ...
    public function __construct($svg, $w = null, $h = null)
    {
        ...
        // If SVG image does not exist, check to make sure the width and height
    ...
    public function setBackgroundColor($r = 0, $g = 0, $b = 0)
    {
        ...
        $rect->addAttribute('fill', 'rgb(' . (int)$r . ',' . (int)$g . ',' . (int)$b . ')');
        return $this;
    }
     ...
     * @return Svg
     */
    public function addGradient(array $color1, array $color2, $type = Svg::HORIZONTAL)
    {
        ...
        if (count($color1) == 3) {
            $c1 = 'rgb(' . (int)$color1[0] . ',' . (int)$color1[1] . ',' . (int)$color1[2] . ')';
        } else {
            $c1 = 'rgb(0,0,0)';
        }

        if (count($color2) == 3) {
            $c2 = 'rgb(' . (int)$color2[0] . ',' . (int)$color2[1] . ',' . (int)$color2[2] . ')';
        } else {
            $c2 = 'rgb(0,0,0)';
        }
        ...
        $stop1->addAttribute('style', 'stop-color: ' . $c1 . '; stop-opacity: 1;');

        $stop2 = $grad->addChild('stop');
        $stop2->addAttribute('offset', '100%');
        $stop2->addAttribute('style', 'stop-color: ' . $c2 . '; stop-opacity: 1;');
